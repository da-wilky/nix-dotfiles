  ...
  instanceType = types.submodule {
    options = {
      ...
      image = mkOption {
        ...
        description =
          "Image to use for instance creation (e.g., images:ubuntu/22.04, images:alpine/edge)";
      ...
      ipAddress = mkOption {
        ...
        default = null;
        description =
          "Static IPv4 address for the instance (CIDR notation, e.g., 10.18.10.50/24). If null, DHCP will be used.";
  ...
  preseedType = types.submodule {
    options = {
      ...
      storageSize = mkOption {
        ...
        default = "50GiB";
        description =
          "Size of the storage pool (for zfs/lvm/btrfs loop devices)";
      };
      ...
      additionalProfiles = mkOption {
        ...
        default = [ ];
        description =
          "Additional profiles to create beyond the default profile";
        example = literalExpression ''
          [
            {
      ...
      additionalStoragePools = mkOption {
        ...
        default = [ ];
        description =
          "Additional storage pools to create beyond the default pool";
        example = literalExpression ''
          [
            {
              name = "fast-storage";
...
in {
  options.myModules.incus = {
    ...
    ui = {
      ...
      host = mkOption {
        ...
        default = "127.0.0.1";
        description =
          "Host address for the Incus web UI to listen on. Use '[::]' for all interfaces (IPv4 and IPv6), '0.0.0.0' for all IPv4, or a specific IP address.";
        example = "0.0.0.0";
      };
  ...
  config = mkIf cfg.enable {
    ...
    users.users = mkMerge
      (map (username: { ${username} = { extraGroups = [ "incus-admin" ]; }; })
        cfg.users);
    ...
    systemd.services = mkMerge (mapAttrsToList (instanceName: instanceConfig:
      let
        ...
        # Separate simple config from multi-line config (like cloud-init)
        simpleConfig =
          filterAttrs (k: v: !(hasInfix "\n" v)) instanceConfig.config;
        multiLineConfig =
          filterAttrs (k: v: hasInfix "\n" v) instanceConfig.config;

        # Build config string from simple attribute set only
        configString = optionalString (simpleConfig != { })
          (concatMapStringsSep " " (key:
            "-c ${escapeShellArg key}=${escapeShellArg simpleConfig.${key}}")
            (attrNames simpleConfig));

        # Build profiles string
        profilesString = concatMapStringsSep " " (profile: "-p ${profile}")
          instanceConfig.profiles;
      ...
      in {
        "incus-instance-${instanceName}" = {
          description =
            "Incus ${toUpper instanceConfig.type}: ${instanceConfig.name}";
          after = [ "incus.service" ];
          ...
          script = ''
            ...
            if ! ${incusCmd} info ${instanceConfig.name} >/dev/null 2>&1; then
              ...
              ${incusCmd} init ${instanceConfig.image} ${instanceConfig.name} \
                --vm=${
                  if instanceConfig.type == "vm" then "true" else "false"
                } \
                ${profilesString} \
                ${configString}
              
              echo "[${instanceConfig.name}] Configuring resources..."
              ${incusCmd} config set ${instanceConfig.name} limits.cpu ${
                toString instanceConfig.cpus
              }
              ...
              ${
                concatMapStringsSep "\n" (key:
                  let value = multiLineConfig.${key};
                  in "${incusCmd} config set ${instanceConfig.name} ${
                    escapeShellArg key
                  } ${escapeShellArg value}") (attrNames multiLineConfig)
              }
              ...
              # Apply simple config options
              ${
                concatMapStringsSep "\n" (key:
                  "${incusCmd} config set ${instanceConfig.name} ${escapeShellArg key} ${
                    escapeShellArg simpleConfig.${key}
                  } || true") (attrNames simpleConfig)
              }
              ...
              ${
                concatMapStringsSep "\n" (key:
                  ...
                  in "${incusCmd} config set ${instanceConfig.name} ${
                    escapeShellArg key
                  } ${escapeShellArg value} || true") (attrNames multiLineConfig)
              }
            ...
            # Configure autostart
            ${incusCmd} config set ${instanceConfig.name} boot.autostart ${
              if instanceConfig.autostart then "true" else "false"
            }
